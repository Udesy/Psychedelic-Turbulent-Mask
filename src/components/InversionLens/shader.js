...
uniform float u_turbulnaceIntensity;
...
vec2 textCoord = vec2(
mix(0.5 - 0.5 / ratio, 0.5 + 0.5 / ratio, ux.x),
uv.y
);
...
flaot jaggedDist = dist + (turbulance(uv * 25.0 + u_time * u_speed) - 0.5)
* u_turbulanceIntensity;

float mask = step(jaggedDist, u_radius);

vec3 finalColor = mix(invertedGray, text.rgb, 1.0 - mask);
